...
- `project_implementation_plan.md` ΓÇö Milestones and implementation roadmap.
- `code_quality_lessons.md` ΓÇö Collected rules and pitfalls from past fixes.
- `session-reports/` ΓÇö Session logs, milestone notes, and knowledge preservation.
...
## ≡ƒÜÇ Quick Start for AI Assistants
...
## ≡ƒöä Development Workflow Standard
...
```python
# Γ¥î WRONG - Creates timezone-naive datetime
timestamp = datetime.utcnow()

# Γ£à CORRECT - Creates timezone-aware datetime
...
```python
# Γ¥î WRONG - FastMCP decorators that don't exist
@app.resource()  # Does not exist
@app.prompt()    # Does not exist

# Γ¥î WRONG - FastMCP attribute that doesn't exist
server.app.tools.keys()  # 'tools' attribute doesn't exist

# Γ£à CORRECT - Only tool decorator is supported
@app.tool()  # All handlers use tool decorator

# Γ£à CORRECT - Access patterns
...
```python
# Γ¥î WRONG - Method doesn't exist
data_store.get_all_events()

# Γ£à CORRECT - Use existing methods
...
## ≡ƒöä Session Management Protocol

### Conversation Limit Monitoring

**After EVERY response, assess if**:
- Estimated remaining interactions Γëñ 5-6 responses
